...
class CompactPlayerWidgetRenderer : OniWidgetRenderer {
    @Composable override fun Render(context: Context, size: WidgetSize, state: OniWidgetPlaybackState, skin: OniSkinTokens) {
        ...
        Box(root.background(ColorProvider(skin.colors.surface)).padding(if (size == WidgetSize.SIZE_4X1) 8.dp else 12.dp)) {
            Box(GlanceModifier.size(if (size == WidgetSize.SIZE_4X4) 170.dp else 110.dp).background(ColorProvider(skin.colors.accentGlow.copy(alpha = 0.18f))).cornerRadius(90.dp).align(Alignment.TopEnd)) {}
            Box(GlanceModifier.fillMaxSize()) {
                when (size) { WidgetSize.SIZE_4X1 -> Small(state, skin, art); WidgetSize.SIZE_4X2 -> Medium(state, skin, art); WidgetSize.SIZE_4X4 -> Large(state, skin, art) }
            }
        }
    }

    @Composable private fun Art(bitmap: Bitmap?, size: Int, skin: OniSkinTokens) {
        Box(GlanceModifier.size((size + 6).dp).background(ColorProvider(skin.colors.accentGlow.copy(alpha = 0.25f))).cornerRadius(((size + 6) / 5).dp).padding(3.dp), Alignment.Center) {
    ...
    @Composable private fun Small(state: OniWidgetPlaybackState, skin: OniSkinTokens, art: Bitmap?) {
        Row(GlanceModifier.fillMaxSize(), verticalAlignment = Alignment.CenterVertically) {
            Art(art, 50, skin); Spacer(GlanceModifier.width(9.dp)); Column(GlanceModifier.defaultWeight()) {
                Text(state.title, maxLines = 1, style = TextStyle(ColorProvider(skin.colors.textPrimary), 13.sp, FontWeight.Medium))
                Text(state.artist, maxLines = 1, style = TextStyle(ColorProvider(skin.colors.textSecondary), 10.sp))
            }
            Spacer(GlanceModifier.width(7.dp)); Control(R.drawable.ic_skip_previous, "Previous", skin, 30, SkipPreviousActionCallback::class.java)
            Spacer(GlanceModifier.width(4.dp)); Control(if (state.isPlaying) R.drawable.ic_pause else R.drawable.ic_play, if (state.isPlaying) "Pause" else "Play", skin, 40, TogglePlayPauseActionCallback::class.java, true)
            Spacer(GlanceModifier.width(4.dp)); Control(R.drawable.ic_skip_next, "Next", skin, 30, SkipNextActionCallback::class.java)
        }
    }
    @Composable private fun Medium(state: OniWidgetPlaybackState, skin: OniSkinTokens, art: Bitmap?) {
        Column(GlanceModifier.fillMaxSize(), verticalAlignment = Alignment.CenterVertically) {
            Row(GlanceModifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) { Art(art, 68, skin); Spacer(GlanceModifier.width(11.dp)); Column(GlanceModifier.defaultWeight()) { Text(state.title, maxLines = 1, style = TextStyle(ColorProvider(skin.colors.textPrimary), 16.sp, FontWeight.Bold)); Text(state.artist, maxLines = 1, style = TextStyle(ColorProvider(skin.colors.accentSecondary), 11.sp, FontWeight.Medium)); Text(if (state.isPlaying) "NOW PLAYING" else "PAUSED", style = TextStyle(ColorProvider(skin.colors.textTertiary), 8.sp, FontWeight.Medium)) } }
            Spacer(GlanceModifier.height(9.dp)); Progress(state, skin); Spacer(GlanceModifier.height(8.dp)); Controls(state, skin, 34, 44)
        }
    }
    @Composable private fun Large(state: OniWidgetPlaybackState, skin: OniSkinTokens, art: Bitmap?) {
        Column(GlanceModifier.fillMaxSize(), horizontalAlignment = Alignment.CenterHorizontally, verticalAlignment = Alignment.CenterVertically) {
            Art(art, 142, skin); Spacer(GlanceModifier.height(10.dp)); Text(state.title, maxLines = 1, style = TextStyle(ColorProvider(skin.colors.textPrimary), 18.sp, FontWeight.Bold)); Text(state.artist, maxLines = 1, style = TextStyle(ColorProvider(skin.colors.accentSecondary), 12.sp, FontWeight.Medium)); Spacer(GlanceModifier.height(10.dp)); Progress(state, skin); Spacer(GlanceModifier.height(8.dp))
