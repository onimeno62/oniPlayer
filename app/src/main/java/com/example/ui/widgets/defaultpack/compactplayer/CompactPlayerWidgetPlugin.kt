...
class CompactPlayerWidgetRenderer : OniWidgetRenderer {
    @Composable override fun Render(context: Context, size: WidgetSize, state: OniWidgetPlaybackState, skin: OniSkinTokens) { val art = WidgetPlaybackStateAdapter.loadArtworkBitmap(context, state.albumArtworkUri); val root = GlanceModifier.fillMaxSize().cornerRadius(24.dp).clickable(actionStartActivity<MainActivity>()); Box(root.background(ColorProvider(skin.colors.surface)).padding(if (size == WidgetSize.SIZE_4X1) 8.dp else 12.dp)) { Box(GlanceModifier.size(if (size == WidgetSize.SIZE_4X4) 170.dp else 110.dp).background(ColorProvider(skin.colors.accentGlow.copy(alpha = .18f))).cornerRadius(90.dp).align(Alignment.TopEnd)) {}; Box(GlanceModifier.fillMaxSize()) { when (size) { WidgetSize.SIZE_4X1 -> Small(state, skin, art); WidgetSize.SIZE_4X2 -> Medium(state, skin, art); WidgetSize.SIZE_4X4 -> Large(state, skin, art) } } } }
    ...
    @Composable private fun Medium(state: OniWidgetPlaybackState, skin: OniSkinTokens, art: Bitmap?) { Column(GlanceModifier.fillMaxSize(), verticalAlignment = Alignment.CenterVertically) { Row(GlanceModifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) { Art(art, 68, skin); Spacer(GlanceModifier.width(11.dp)); Column(GlanceModifier.defaultWeight()) { Text(state.title, maxLines = 1, style = TextStyle(ColorProvider(skin.colors.textPrimary), 16.sp, FontWeight.Bold)); Text(state.artist, maxLines = 1, style = TextStyle(ColorProvider(skin.colors.accentSecondary), 11.sp, FontWeight.Medium)); Text(if (state.isPlaying) "NOW PLAYING" else "PAUSED", style = TextStyle(ColorProvider(skin.colors.textTertiary), 8.sp, FontWeight.Medium)) } }; Spacer(GlanceModifier.height(9.dp)); Progress(state, skin, 220); Spacer(GlanceModifier.height(8.dp)); Controls(state, skin, 34, 44) } }
    @Composable private fun Large(state: OniWidgetPlaybackState, skin: OniSkinTokens, art: Bitmap?) { Column(GlanceModifier.fillMaxSize(), horizontalAlignment = Alignment.CenterHorizontally, verticalAlignment = Alignment.CenterVertically) { Art(art, 142, skin); Spacer(GlanceModifier.height(10.dp)); Text(state.title, maxLines = 1, style = TextStyle(ColorProvider(skin.colors.textPrimary), 18.sp, FontWeight.Bold)); Text(state.artist, maxLines = 1, style = TextStyle(ColorProvider(skin.colors.accentSecondary), 12.sp, FontWeight.Medium)); Spacer(GlanceModifier.height(10.dp)); Progress(state, skin, 260); Spacer(GlanceModifier.height(8.dp)); Row(verticalAlignment = Alignment.CenterVertically) { Control(R.drawable.ic_shuffle, "Shuffle", skin, 34, ToggleShuffleActionCallback::class.java, state.isShuffle); Spacer(GlanceModifier.width(10.dp)); Controls(state, skin, 38, 48); Spacer(GlanceModifier.width(10.dp)); Control(R.drawable.ic_repeat, "Repeat", skin, 34, ToggleRepeatActionCallback::class.java, state.isRepeat) } } }
    @Composable private fun Progress(state: OniWidgetPlaybackState, skin: OniSkinTokens, width: Int) { val f = if (state.durationMs > 0) (state.positionMs.toFloat() / state.durationMs).coerceIn(0f, 1f) else 0f; Box(GlanceModifier.fillMaxWidth().height(5.dp).background(ColorProvider(skin.colors.surfaceVariant.copy(alpha = .7f))).cornerRadius(3.dp)) { if (f > 0) Box(GlanceModifier.width((width * f).coerceAtLeast(2f).dp).height(5.dp).background(ColorProvider(skin.colors.primary)).cornerRadius(3.dp)) } }
